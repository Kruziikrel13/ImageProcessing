...
void TIFFimage::open(const std::string &n,
                     const char mode) throw(IMAGE::file_io_failed) {

  imageFile_m = TIFFOpen(n.c_str(), &mode);
  // failed to open
  if (!imageFile_m)
    throw IMAGE::file_io_failed("Could not open file " + n + " in mode " +
                                mode);
...
void TIFFimage::readImageRaster() throw(IMAGE::bad_alloc,
                                        IMAGE::image_format_error,
                                        IMAGE::empty_image) {
  ...
  if (!TIFFReadRGBAImage(imageFile_m, raster.getWidth(), raster.getHeight(),
                         (unsigned int *)ptr, 0))
    throw IMAGE::image_format_error("TIFF image internal error   " + name_m);
}

void TIFFimage::writeRasterToImage() throw(IMAGE::image_format_error,
                                           IMAGE::empty_raster) {
  ...
  TIFFSetField(imageFile_m, TIFFTAG_ROWSPERSTRIP,
               TIFFDefaultStripSize(imageFile_m, temp_width * 4));
  ...
  for (unsigned int row = 0; row < temp_height; row++) {
    ...
    if (TIFFWriteScanline(
            imageFile_m,
            &raster_m[(temp_height - row - 1) * temp_width * temp_samples], row,
            0) < 0) {
      throw IMAGE::image_format_error("TIFF image internal error   " + name_m);
    }
  }
