...
void PNGimage::open(const std::string &name,
                    const char mode) throw(IMAGE::image_format_error,
                                           file_io_failed) {
...
void PNGimage::readImageRaster() throw(IMAGE::bad_alloc,
  ...
  cilk_for(unsigned int i = 0; i < height; i++) {
    memcpy(&ptr[((height - 1) - i) * width * samples], row_pointers[i],
           width * samples);
    free(row_pointers[i]);
  }
...
void PNGimage::writeRasterToImage() throw(IMAGE::image_format_error,
                                          IMAGE::empty_raster) {
...
#ifdef CILK_PARALLEL
  cilk_for(unsigned int i = 0; i < height; i++) {
    memcpy(row_pointers[i], &ptr[((height - 1) - i) * width * samples],
           width * samples);
  }
