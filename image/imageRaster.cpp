...
ImageRaster::ImageRaster() throw()
    : width_m(0), height_m(0), samples_per_pixel_m(0), total_pixels_m(0),
      raster_m(NULL), has_raster_m(false) {
#ifdef DEBUG
  counter.increase();
#endif
}
...
void ImageRaster::copyRaster(const ImageRaster &copy) throw() {
  ...
  if (copy.hasRaster()) {
    ...
    cilk_for(unsigned int i = 0; i < height_m; i++)
        memcpy(&raster_m[i * width_m * samples_per_pixel_m],
               &copy.raster_m[i * width_m * samples_per_pixel_m],
               width_m * samples_per_pixel_m);
#endif
...
void ImageRaster::createRaster(const ImageRaster &copy) throw(
    IMAGE::bad_alloc, IMAGE::empty_raster) {
...
void ImageRaster::createRaster(
    ...
    const unsigned int s) throw(IMAGE::bad_alloc, IMAGE::invalid_argument) {
