...
Arguments ImageRaster and integer for percentage adjustment of brightness -100%
< factor <= 100% from every component of every pixel.*/
...
#pragma omp parallel for
  for (unsigned int i = 0;
    ...
    else if (raster.raster_m[i + 2] > highLimit)
      raster.raster_m[i + 2] = 255;
  }
}
...
void zoomImage(IMAGE::ImageRaster &raster, unsigned int factor) {
  ...
  if (factor > 0 && factor <= 100) {
    ...
    factor = 10 - factor;
    // factor /= 100/factor;
    zoomImage(raster, raster.width_m / factor, raster.height_m / factor,
              raster.width_m - raster.width_m / factor,
              raster.height_m - raster.height_m / factor);
  }
}
...
#pragma omp parallel shared(factor, intPart, fraqPart)                         \
    firstprivate(E, prevSource, source, destination)
  {
    ...
    // unsigned char* prevSource = NULL;
    source = &raster.raster_m[((unsigned int)(j * factor) * raster.width_m *
                               raster.samples_per_pixel_m)];
    destination =
        &tempRaster
    ...
    for (unsigned int col = 0; col < raster.width_m; col++)
      ...
      for (unsigned int i = 0; i < raster.samples_per_pixel_m; i++) {
        ...
        /*if the current pixel is in posiotion that the table step*step can be
          computed normal r , c , end_r , end_c are using the default values if
          the pixel is at the edge of the image this varaibles must be computed
          in order not to exit the corners of the image
        */
        // holds the start row and start column for the table step*step for
        // every pixel
        signed int r = factor, c = factor;
        ////holds the end row and end column of the table step*step for every
        ///pixel
...
#pragma omp parallel for private(factor)
  for (unsigned int i = 0;
    ...
    factor = raster.raster_m[i] * 0.393 + raster.raster_m[i + 1] * 0.769 +
