...
static void rotate180(IMAGE::ImageRaster &raster) {
  ...
  for (unsigned int firstRow = 0, lastRow = height - 1;
       firstRow < height && lastRow >= 0; firstRow++, lastRow--)
...
static void rotateLeft(IMAGE::ImageRaster &raster) {
  ...
  for (unsigned int row = 0, tempColumn = height;
       row < height && tempColumn > 0; row++, tempColumn--)
...
static void rotateRight(IMAGE::ImageRaster &raster) {
  ...
  for (unsigned int tempRow = 0, column = width; tempRow < width && column > 0;
       tempRow++, column--)
