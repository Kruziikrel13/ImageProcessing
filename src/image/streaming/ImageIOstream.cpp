...
namespace IMAGE {
    ...
    void ImageIOstream::setReadPath(std::string s) {
        readPath = s;
    }

    std::string ImageIOstream::getWritePath() const {
        return writePath;
    }

    void ImageIOstream::setWritePath(std::string s) {
        writePath = s;
    }

    std::string ImageIOstream::getOperation() const {
        return operation;
    }

    void ImageIOstream::setOperation(std::string s) {
        operation = s;
    }
...
#pragma omp single nowait
    ...
    /* reader worker.The worker opens one image a time reads it and puts the raster
of that image to the list for process and then goes to open the next image*/
...
#pragma omp critical(OUTPUT)
    ...
    // processor worker.Takes the pointers from the read list processes them and
    // pushes them to the write list for the writer
...
#pragma omp critical(OUTPUT)
    ...
    /*the writer worker is responsible for writing the raster to the image and
 * closing the image and destroying the image*/
